...
function getTotalCaloriesFromMeals(){
  ...
  mealTables.forEach(function(mealTable){
    mealCalories = parseInt($(mealTable).children('tfoot').children().children('.total-calories').text())
    totalCalories += mealCalories;
    daily.push(goalCalories(mealTable));
    consumed.push(mealCalories);
    mealName.push($(mealTable).attr('id'));
  });
  saveFoodList('d3Daily', daily);
  saveFoodList('d3Consumed', consumed);
  saveFoodList('d3MealName', mealName);
