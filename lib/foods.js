...
function loadFoodList(whichTable) {
  if (whichTable.attr('id') == "food-list" || whichTable.attr('id') == "foods-for-meals") {
    ...
    var total = totalCalories(foods);
    var remaining = remainingCalories(whichTable, total);
...
function colorRemainingCalories(whichTable, remaining){
  if (remaining < 0) {
    ...
    $(whichTable).children('tfoot').children().children('.remaining-calories').css('color', 'lightgreen');
  }
};
...
function updateMealLists(updatedObject){
  updateAMeal('breakfast', updatedObject);
  updateAMeal('lunch', updatedObject);
  updateAMeal('snack', updatedObject);
  updateAMeal('dinner', updatedObject);
}

function updateAMeal(meal, updatedObject){
  var breakfastFoods = loadFoods(meal);
  var idIncluded = $.each(breakfastFoods, function(oneFood) { oneFood.id == updatedObject.id; });
  if (idIncluded) {
    updateFoodList(meal, updatedObject);
  };
};
...
function getFoodsFromIds(idCollection){
  var allFoods = loadFoods('all-foods');
  var selectedFoods = $.grep(allFoods, function(oneFood){
